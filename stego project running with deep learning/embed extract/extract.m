function extract(input_image_path)
    file_name = 'F:\stego project running\text\extract_bin.txt';
    dec_file_name = 'F:\stego project running\text\convert_dec.txt';
    % Read stego image
    stego_image = imread(input_image_path);

    % convert to YCbCr color space
    stego_ycbcr = rgb2ycbcr(stego_image);
    ...
    % apply DWT to the stego image, on Cb channel
    % L/H = Low/High
    % LL = approximation coefficients
    % HL/LH/HH = detail coefficients
    [~, HL, ~, HH] = lwt2(double(cb), haar_wavelet);
